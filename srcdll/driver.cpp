  ...
  HANDLE module, DWORD code, void* resv)
{
  switch (code) {
	...
	(GetModuleHandle(L"coredll"), L"DisableThreadLibraryCalls");
        if(proc) (*proc)(module);
      };
      break;
  ...
  PCWCH dllname)
{
  ...
  , L"Drivers\\USB\\LoadClients\\Default\\Default\\Default\\" CLIENTNAME
   ...
   DWORD resv)
{
  ...
  *accept= FALSE;
  ...
  if(sets->dwVendorId!=USB_NO_INFO) {
    ...
    if(sets->dwProductId!=USB_NO_INFO) {
      ...
      if(sets->dwReleaseNumber!=USB_NO_INFO) sp+= wsprintf(sp, L"_%u", sets->dwReleaseNumber);
    };
  } else sp+= wsprintf(sp, L"\\Default");
  if(sets->dwDeviceClass!=USB_NO_INFO) {
    ...
    if(sets->dwDeviceSubClass!=USB_NO_INFO) {
      ...
      if(sets->dwDeviceProtocol!=USB_NO_INFO) sp+= wsprintf(sp, L"_%u", sets->dwDeviceProtocol);
    };
  } else sp+= wsprintf(sp, L"\\Default");
  if(sets->dwInterfaceClass!=USB_NO_INFO) {
    ...
    if(sets->dwInterfaceSubClass!=USB_NO_INFO) {
      ...
      if(sets->dwInterfaceProtocol!=USB_NO_INFO) sp+= wsprintf(sp, L"_%u", sets->dwInterfaceProtocol);
    };
  } else sp+= wsprintf(sp, L"\\Default");
  ...
  if(sets->dwVendorId==USB_NO_INFO&&sets->dwDeviceClass==USB_NO_INFO&&sets->dwInterfaceClass==USB_NO_INFO) {
    ...
    if(rt==REG_SZ) dllname[rd/sizeof(WCHAR)]= 0/*guard*/; else dllname[0]= 0;
    ...
    rd= sizeof(instmode); rt= 0;
    if(RegQueryValueEx(hk, L"InstallMode", 0, &rt, (BYTE*)&instmode, &rd)) rt= 0;
    if(rt==REG_DWORD) {} else instmode= 0;
    RegDeleteValue(hk, L"Dll");
    RegDeleteValue(hk, L"InstallTime");
    RegDeleteValue(hk, L"InstallMode");
    RegCloseKey(hk);
    if(instmode==0&&instime>=3000||dllname[0]==0) return FALSE; //not loaded

    wsprintf(ckey, L"Drivers\\USB\\LoadClients\\%u_%u_%u\\Default\\Default\\%s"
    ...
    classaddr= 0; recvaddr= 0; sendaddr= 0;
    int recvw2= 0; int sendw2= 0;
    int recvaddr2= 0; int sendaddr2= 0;
	...
	USB_ENDPOINT_DESCRIPTOR const* es= 0;
	  ...
	  if((ue->Descriptor.bEndpointAddress&128)&&(ue->Descriptor.bmAttributes&3)==3) { //interrupt in
	    if(ec) ee++;
	    ec= &ue->Descriptor;
          } else if((ue->Descriptor.bEndpointAddress&128)&&(ue->Descriptor.bmAttributes&3)==2) { //bulk in
	    if(er) ee++;
	    er= &ue->Descriptor;
	  } else if((ue->Descriptor.bEndpointAddress&128)==0&&(ue->Descriptor.bmAttributes&3)==2) { //bulk out
	    if(es) ee++;
	    es= &ue->Descriptor;
	  };
	};
	if(ee==0) {
          if(ec) {
            int w= 1;
	    if((ec->wMaxPacketSize&0x7ff)>=10) w+= 2;
	    if(er&&es&&ua->Descriptor.bNumEndpoints==3) w+= 1;
	  ...
	  if(er) {
	    ...
	    if(es) {
	      w+= 1;
	      if((er->bEndpointAddress^es->bEndpointAddress)==128) w+= 1; //same endpoint number
	      if(ua->Descriptor.bNumEndpoints==2) w+= 2;
	    };
	    if(ua->Descriptor.bInterfaceClass==10) {
	      ...
	      if(curcdc==11) w+= 5;
            ...
            if(recvw<w) {
	      recvw2= recvw; recvaddr2= recvaddr;
	      recvw= w;
	  ...
	  if(es) {
	    ...
	    if(ua->Descriptor.bInterfaceClass==10) {
	      ...
	      if(curcdc==11) w+= 5;
            ...
            if(sendw<w) {
	      sendw2= sendw; sendaddr2= sendaddr;
	      sendw= w;
    ...
    usbmode= MODE_BASIC;
    if(classw>=20&&recvw2>=20&&sendw2>=20) { //data port found...
      usbmode= MODE_WMC;
      sendaddr|= sendaddr2<<8;
      recvaddr|= recvaddr2<<8;
    } else if(classaddr) {
      usbmode= MODE_CDC;
      if(info->lpActiveConfig->dwNumInterfaces==1&&classaddr==129&&recvaddr==131&&sendaddr==2) usbmode|= MODE_2303;
    };
    ...
    if(info->Descriptor.idVendor==0x054c&&info->Descriptor.idProduct==0x006c) usbmode= MODE_RCS;
    //AH-K3001V sometimes stall when SET_LINE_CODING is sent
    if(info->Descriptor.idVendor==0x0482&&info->Descriptor.idProduct==0x0203) usbmode|= MODE_NOBAUD;
    ...
    if(rt==0) rt= info->Descriptor.iManufacturer;
    if(rt!=0) {
      //HPW-600JC does not allow USB_SHORT_TRANSFER in IssueVendorTransfer,
      //so we get size first.
#if 1
      USB_TRANSFER ut= uf->lpGetDescriptor(uh, 0, 0
      , 0, USB_STRING_DESCRIPTOR_TYPE, (BYTE)rt, 0x409, 2, dllname);
#else
      ...
      USB_TRANSFER ut= uf->lpIssueVendorTransfer(uh, 0, 0
      , USB_IN_TRANSFER|USB_SEND_TO_DEVICE|USB_SHORT_TRANSFER_OK, &req, dllname, 0);
#endif
      if(ut) {
        rd= 0;
        uf->lpGetTransferStatus(ut, &rd, 0);
        uf->lpCloseTransfer(ut);
	if(!(rd==2&&*(BYTE*)dllname<=sizeof(dllname)-sizeof(WCHAR))) rt= 0;
      } else rt= 0;
      if(rt) {
#if 1
        ut= uf->lpGetDescriptor(uh, 0, 0
        , 0, USB_STRING_DESCRIPTOR_TYPE, (BYTE)rt, 0x409, *(BYTE*)dllname, dllname);
#else
    ...
    if(rt==0) wcscpy(dllname+1, L"RS232 USB");
    WCHAR* rs= 0;
    if((usbmode&0xff)==MODE_RCS) {
      ...
      if(usbmode&MODE_2303) rs= L",pl2303";
      else rs= L",cdc";
    ...
    if(rs&&rt+wcslen(rs)<numof(dllname)) wcscpy(dllname+rt, rs);
  ...
  if(usbmode==0x100) usbmode= MODE_CDC; //old-mode
  ...
  if(RegQueryValueEx(hk, L"Priority256", 0, &rt, (BYTE*)&priority, &rd)) rt= 0;
  if(rt!=REG_DWORD) priority= 0;
  rd= sizeof(recvsize); rt= 0;
  if(RegQueryValueEx(hk, L"ReceiveBuffer", 0, &rt, (BYTE*)&recvsize, &rd)) rt= 0;
  ...
  switch(usbmode&0xff) {
  case MODE_BASIC: dx= new device_extension; break;
  case MODE_CDC: dx= new cdc_extension; break;
  case MODE_WMC: dx= new wmc_extension; break;
  case MODE_FTDI:
    if(usbmode&MODE_8U232) dx= new ftdi_extension;
    else dx= new ft100_extension;
    break;
  case MODE_RCS: dx= new rcs_extension; break;
  };
  ...
  *accept= TRUE;
  ...
  if(dx->initbase()==0) return TRUE;  //device connected, but do not activate the device
